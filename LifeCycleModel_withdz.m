...
% One stochastic exogenous state variable: z, an exogenous shock that takes two possible values, 1 and 0, representing employment and unemploymnet respectively.
% Age: j
...
% Grid sizes to use
n_d=201; % Endogenous labour choice (fraction of time worked)
...
timeplainvanilla=toc

level1ii=round(linspace(1,n_a,vfoptions.level1n));

% N_j
max(max(abs(V(:,:,end)-V2(:,:,end))))
max(max(abs(Policy(1,:,:,end)-Policy2(1,:,:,end))))
max(max(abs(Policy(2,:,:,end)-Policy2(2,:,:,end))))

max(max(abs(V(level1ii,:,end)-V2(level1ii,:,end)))) % first level
max(max(max(abs(Policy(:,level1ii,:,end)-Policy2(:,level1ii,:,end))))) % first level

% N_j-1
max(max(abs(V(:,:,end-1)-V2(:,:,end-1))))
max(max(abs(Policy(1,:,:,end-1)-Policy2(1,:,:,end-1))))
max(max(abs(Policy(2,:,:,end-1)-Policy2(2,:,:,end-1))))

max(max(abs(V(level1ii,:,end-1)-V2(level1ii,:,end-1)))) % first level
max(max(max(abs(Policy(:,level1ii,:,end-1)-Policy2(:,level1ii,:,end-1))))) % first level


Policy(:,1:21,:,end-1) % chooses assets=1
Policy2(:,1:21,:,end-1)
abs(V(1:21,:,end-1)-V2(1:21,:,end-1))
V2(1:21,:,end-1)

% N_j-2
max(max(abs(V(:,:,end-2)-V2(:,:,end-2))))
max(max(abs(Policy(1,:,:,end-2)-Policy2(1,:,:,end-2))))
max(max(abs(Policy(2,:,:,end-2)-Policy2(2,:,:,end-2))))

max(max(abs(V(level1ii,:,end-2)-V2(level1ii,:,end-2)))) % first level
max(max(max(abs(Policy(:,level1ii,:,end-2)-Policy2(:,level1ii,:,end-2))))) % first level

% Earlier periods
for jj=31:40
    max(max(max(abs(Policy(:,level1ii,:,end-jj)-Policy2(:,level1ii,:,end-jj))))) % first level
end

max(max(max(abs(Policy(:,level1ii,:,end-36)-Policy2(:,level1ii,:,end-36))))) % first level
max(max(max(abs(Policy(:,level1ii,:,end-37)-Policy2(:,level1ii,:,end-37))))) % first level
max(max(max(abs(Policy(:,level1ii,:,end-38)-Policy2(:,level1ii,:,end-38))))) % first level
max(max(max(abs(Policy(:,level1ii,:,end-39)-Policy2(:,level1ii,:,end-39))))) % first level
N_j-37
Params.Jr


Policy(1,level1ii,:,end-37)-Policy2(1,level1ii,:,end-37)
Policy(2,level1ii,:,end-37)-Policy2(2,level1ii,:,end-37)

Policy(2,level1ii,:,end-37)
Policy2(2,level1ii,:,end-37)
% So expection of V' is 

Policy2(1,level1ii,:,end-37)


% Final
max(max(abs(V(:)-V2(:))))
max(max(abs(Policy(:)-Policy2(:))))



% Level 1
max(max(max(abs(V(level1ii,:,:)-V2(level1ii,:,:)))))
max(max(max(max(abs(Policy(:,level1ii,:,:)-Policy2(:,level1ii,:,:))))))

max(max(max(max(abs(Policy(:,level1ii,1,:)-Policy2(:,level1ii,1,:))))))
max(max(max(max(abs(Policy(:,level1ii,2,:)-Policy2(:,level1ii,2,:)))))) % error is in the second z for some reason
